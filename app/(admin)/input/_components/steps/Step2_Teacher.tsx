    ...
    return (
        <div className="flex flex-col gap-4 mt-4">
            ...
            {data.teacherFile ? (
                <div className="flex flex-col gap-2">
                    <div className={`flex items-center justify-between p-4 border rounded-xl ${validationErrors.length > 0 ? 'bg-red-50 border-red-200' : 'bg-blue-50 border-blue-100'}`}>
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-sm font-medium text-gray-900">{data.teacherFile.name}</p>
                                <p className="text-xs text-gray-500">{(data.teacherFile.size / 1024).toFixed(1)} KB</p>
                            </div>
                        </div>
                ...
                <FileDropzone
                    onFileSelect={(file) => onChange('teacherFile', file)}
