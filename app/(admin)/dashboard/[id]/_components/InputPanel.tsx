...
import { ALL_TAB_NAMES, extractSheetId, filenameToTabName, fetchAllSheetTabs } from '../_utils/csvHelpers';
...
export default function InputPanel({
  ...
  return (
    <div data-testid="input-panel" className="rounded-xl border-2 border-dashed border-primary-border bg-primary-light overflow-hidden">
      {/* Header */}
              ...
              <div className="space-y-3 bg-surface border border-border rounded-xl p-4">
                ...
                {linkStatus === 'success' && (
                  <div className="space-y-1 text-xs">
                    {linkFound.length > 0 && <p className="text-green-700">✅ พบ: {linkFound.join(', ')}</p>}
                    {linkMissing.length > 0 && <p className="text-orange-600">⚠️ ไม่พบ: {linkMissing.join(', ')}</p>}
                  </div>
                )}
              </div>
            ) : (
