...
const VerifyBox: React.FC = () => {
    ...
    return (
        <div className="bg-surface rounded-2xl shadow-xl p-8 w-[450px] min-h-[500px] flex flex-col relative">
            <button
                onClick={() => router.push('/login')}
                className="absolute top-6 left-6 text-foreground-muted hover:text-foreground-muted transition-colors"
            >
                <ArrowLeft size={24} />
            </button>
            ...
            <div className="flex-1 flex flex-col items-center mt-6">
                ...
                <p className="text-foreground-muted text-center text-sm mb-8 px-4">
                ...
                <form onSubmit={handleSubmit} className="w-full">
                    <div className="flex justify-between gap-2 mb-8">
                        {otp.map((digit, index) => (
                            <input
                                ...
                                className={`w-12 h-14 text-center text-2xl font-bold border-2 rounded-xl focus:border-primary focus:ring-2 focus:ring-primary-light/50 outline-none transition-all ${error ? 'border-red-300 bg-red-50' : digit ? 'border-primary text-primary bg-primary-light' : 'border-border bg-surface'
                    ...
                    <button
                        ...
                        className={`w-full py-3.5 rounded-xl font-medium transition-all flex items-center justify-center gap-2 ${isSuccess
                                ...
                                : 'bg-primary hover:bg-primary-hover text-white disabled:opacity-50 disabled:bg-primary'
                            }`}
                    >
                        {loading ? (
                            <>
