...
import { getRoomSchedule, ScheduleItem, ScheduleData, ROOM_CODES, ROOM_META } from '@/app/(admin)/schedule/_utils/dummyData';
...
export default function RoomSchedulePage() {
  ...
  const [roomCode, setRoomCode] = useState(ROOM_CODES[0]);
  ...
  useEffect(() => {
    setScheduleData(getRoomSchedule(roomCode));
  }, [roomCode]);
  ...
  const roomMeta = ROOM_META[roomCode];
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className={`px-2 py-0.5 rounded text-[11px] font-semibold ${
                roomMeta?.type === 'specialist'
                  ? 'bg-purple-100 text-purple-700'
                  : 'bg-blue-100 text-blue-700'
              }`}>
                {roomMeta?.type === 'specialist' ? 'Specialist' : 'Homeroom'}
              </span>
            </div>
          </div>
